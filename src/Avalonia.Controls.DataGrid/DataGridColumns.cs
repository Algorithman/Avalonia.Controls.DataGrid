...
namespace Avalonia.Controls
{
    ...
    partial class DataGrid
    {
        ...
        /// <summary>
        /// Defines an Avalonia styled property for the <see cref="ColumnTypeSelector"/>.
        /// This allows the column type selector to be set via XAML or styles and supports Avalonia property system features.
        /// </summary>
        public static readonly StyledProperty<IDataGridColumnTypeSelector> ColumnTypeSelectorProperty =
            AvaloniaProperty.Register<DataGrid, IDataGridColumnTypeSelector>(
                nameof(ColumnTypeSelector));

        /// <summary>
        /// Gets or sets the <see cref="IDataGridColumnTypeSelector"/> used by this DataGrid instance
        /// to determine which <see cref="DataGridBoundColumn"/> subclass to create for each property.
        /// Can be set in code or via XAML using the Avalonia property system.
        /// </summary>
        public IDataGridColumnTypeSelector ColumnTypeSelector
        {
            get => GetValue(ColumnTypeSelectorProperty);
            set => SetValue(ColumnTypeSelectorProperty, value);
        }
        ...
        private static void RefreshCellElement(DataGridColumn dataGridColumn, DataGridRow dataGridRow, string propertyName)
        {
            ...
            {
                dataGridColumn.RefreshCellContentInternal(element, propertyName);
        ...
        private DataGridBoundColumn GetDataGridColumnFromType(Type type, string propertyName)
        {
            Debug.Assert(type != null);

            // run custom type selector first
            var column = ColumnTypeSelector?.SelectColumn(type, propertyName);
            if (column != null)
                return column;
            ...
            if (type == typeof(bool))
                ...
                return new DataGridCheckBoxColumn { IsThreeState = true };
        ...
        private DataGridAutoGeneratingColumnEventArgs GenerateColumn(Type propertyType, string propertyName, string header)
        {
            // Create a new DataBoundColumn for the Property
            DataGridBoundColumn newColumn = GetDataGridColumnFromType(propertyType, propertyName);
